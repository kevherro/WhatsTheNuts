...
extension NutsController {
  private func nuts() -> HandResult {
    ...
    var strongestHandResult: HandResult = (
      communityStrength.strength, communityCards
    )
  ...
  private func evaluate(_ cards: [CardModel]) -> HandResult {
    ...
    if let cards = isRoyalFlush(bitmask: bitmask, cards: cards) {
      return (.royalFlush, cards)
    }
    if let cards = isStraightFlush(bitmask: bitmask, cards: cards) {
      return (.straightFlush, cards)
    }
    if let cards = isFourOfAKind(bitmask: bitmask, cards: cards) {
      return (.fourOfAKind, cards)
    }
    if let cards = isFullHouse(bitmask: bitmask, cards: cards) {
      return (.fullHouse, cards)
    }
    if let cards = isFlush(bitmask: bitmask, cards: cards) {
      return (.flush, cards)
    }
    if let cards = isStraight(bitmask: bitmask, cards: cards) {
  ...
  fileprivate func isRoyalFlush(bitmask: UInt64, cards: [CardModel])
    -> [CardModel]?
  {
    let clubsRoyalFlush: UInt64 =
    ...
    if bitmask & clubsRoyalFlush == clubsRoyalFlush {
      return cards.filter {
        royalRanks.contains($0.rank.rawValue) && $0.suit == .clubs
      }
    } else if bitmask & diamondsRoyalFlush == diamondsRoyalFlush {
      return cards.filter {
        royalRanks.contains($0.rank.rawValue) && $0.suit == .diamonds
      }
    } else if bitmask & heartsRoyalFlush == heartsRoyalFlush {
      return cards.filter {
        royalRanks.contains($0.rank.rawValue) && $0.suit == .hearts
      }
    } else if bitmask & spadesRoyalFlush == spadesRoyalFlush {
      return cards.filter {
        royalRanks.contains($0.rank.rawValue) && $0.suit == .spades
      }
    }
  ...
  fileprivate func isStraightFlush(bitmask: UInt64, cards: [CardModel])
    -> [CardModel]?
  {
    for i in 0..<4 {
      var straightFlushMask: UInt64 =
        (1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4)
      for j in 0...8 {
        ...
        if bitmask & currentMask == currentMask {
          ...
          for k in j...(j + 4) {
            ...
            if let card = cards.first(where: {
              $0.rank.rawValue == rank && $0.suit == suit
            }) {
              straightFlushCards.append(card)
            }
          }
          return straightFlushCards.sorted(by: {
  ...
  fileprivate func isFourOfAKind(bitmask: UInt64, cards: [CardModel])
    -> [CardModel]?
  {
    for i in 0..<13 {
      let mask: UInt64 = 1 << i
  ...
  fileprivate func isFullHouse(bitmask: UInt64, cards: [CardModel])
    -> [CardModel]?
  {
      ...
      if count == 3 {
        ...
        for j in 0..<4 {
          if bitmask & (mask << (j * 13)) != 0 {
            if let card = cards.first(where: {
              $0.rank.rawValue == i && $0.suit.rawValue == j
            }) {
      ...
      if count >= 5 {
        ...
        for j in 0..<13 {
          if bitmask & (1 << (j + 13 * i)) != 0 {
            if let card = cards.first(where: {
              $0.rank.rawValue == j && $0.suit.rawValue == i
            }) {
              flushCards.append(card)
            }
          }
        }
        return flushCards.sorted(by: { $0.rank.rawValue > $1.rank.rawValue })
          .prefix(5)
          .map { $0 }
      }
    }
  ...
  fileprivate func isStraight(bitmask: UInt64, cards: [CardModel])
    -> [CardModel]?
  {
    // Generate a mask that only represents ranks without considering suits
    let rankMask: UInt64 =
      bitmask | (bitmask >> 13) | (bitmask >> 26) | (bitmask >> 39)

    // Represents the 5-card sequence A2345
    var straightMask: UInt64 =
      (1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4)
    ...
    for _ in 0...8 {
      if (rankMask & straightMask) == straightMask {
        ...
        if straightCards.count == 5 {
          return straightCards.sorted(by: {
            $0.rank.rawValue < $1.rank.rawValue
          })
        }
      }
    ...
    let highStraightMask: UInt64 =
      (1 << 9) | (1 << 10) | (1 << 11) | (1 << 12) | (1 << 0)
    if (rankMask & highStraightMask) == highStraightMask {
      ...
      if highStraightCards.count == 5 {
        return highStraightCards.sorted(by: {
          $0.rank.rawValue < $1.rank.rawValue
        })
      }
    }
  ...
  fileprivate func isThreeOfAKind(bitmask: UInt64, cards: [CardModel])
    -> [CardModel]?
  {
    for i in 0..<13 {
      let mask: UInt64 = 1 << i
      ...
      if count == 3 {
        ...
        let additionalCards = cards.filter { $0.rank.rawValue != i }.sorted(
          by: {
            $0.rank.rawValue > $1.rank.rawValue
          }).prefix(2)
  ...
  fileprivate func isTwoPair(bitmask: UInt64, cards: [CardModel])
    -> [CardModel]?
  {
    var pairsFound = 0
    var pairRanks: [Int] = []
  ...
  fileprivate func isOnePair(bitmask: UInt64, cards: [CardModel])
    -> [CardModel]?
  {
    for i in 0..<13 {
      let mask: UInt64 = 1 << i
...
struct RoundController {
  ...
  init(communityCards: [CardModel]? = nil) {
    ...
    if let providedCommunityCards = communityCards,
      providedCommunityCards.count <= 5
    {
